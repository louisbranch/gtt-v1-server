...
exports.updateStatus = function(id, params, date, callback){
  ...
  if (params.start) {
    start = {start: params.start, tasks: []}; // Validate time format
    insert(project, start, date, callback);
  } else if (params.end) {
    get(project, date, function(err, day){
      if (err) { return callback({error: true, message: err.message}); }
...
var finish = function(project, params, day, callback){
  var end = {end: params.end}
  var dayUpdated = merge(day, end);
